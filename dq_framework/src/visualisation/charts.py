...
def missingness_heatmap(profile: DatasetProfile, output_dir: str) -> str:
    ...
    # Scale height: more rows for large datasets, capped at 10
    h = min(10, max(4, n * 0.45))
    # Scale font: smaller for large datasets so labels fit
    label_fs = max(8, min(11, int(200 / n)))
    ...
    ax.set_xticks([])
    ax.set_title("1. Missingness per Column", loc="left", fontsize=12)
    ax.set_xlabel("← Complete       Missing →", fontsize=11, color=MID)

    cbar = plt.colorbar(im, ax=ax, fraction=0.025, pad=0.02)
    cbar.set_label("% missing", fontsize=11)
    cbar.ax.tick_params(labelsize=10)

    # Only annotate if dataset is small enough to read
    if n <= 30:
        for i, p in enumerate(pct):
            if p > 0:
                color = "white" if p > 50 else DARK
                ax.text(0, i, f"  {p:.0f}%", va="center",
                        fontsize=max(8, label_fs - 1), color=color)

    fig.tight_layout()
    return _save(fig, out, "1_missingness_heatmap.png")


# ── 2. Dimension radar — grey fill, coloured dots ────────────────────────────
def dimension_radar(profile: DatasetProfile, output_dir: str) -> str:
    ...
    # Grey fill — shape carries no quality meaning
    ...
    # Coloured dots per IBM Carbon threshold
    for angle, score in zip(angles, scores):
        ...
        # Push label outward to avoid boundary overlap
        offset = 0.15 if score > 0.7 else -0.15
        ax.text(angle, min(score + offset, 0.95), f"{score:.2f}",
                ha="center", va="center", fontsize=11,
                fontweight="bold", color=dot_color)

    ax.set_thetagrids(np.degrees(angles), dims, fontsize=11)
    ax.set_ylim(0, 1)
...
def column_scores_bar(profile: DatasetProfile, output_dir: str) -> str:
    ...
    # Scale height and font for large datasets
    h        = min(12, max(4, n * 0.42))
    label_fs = max(8, min(11, int(200 / n)))
    ...
    ax.set_title("3. Column Quality Scores", loc="left", fontsize=12)
    ...
    # Score labels — only if there's space
    for bar, score in zip(bars, scores):
        ax.text(score + 0.01, bar.get_y() + bar.get_height() / 2,
                f"{score:.2f}", va="center",
                fontsize=max(8, label_fs - 1), color=DARK)
...
def issue_severity_summary(profile: DatasetProfile, output_dir: str) -> str:
    ...
    ax.set_title("4. Issues by Severity", loc="left", fontsize=12)
...
def boxplots(profile: DatasetProfile, output_dir: str) -> str:
    ...
    # Scale width for many numeric columns
    w = min(10, max(CHART_W, n * 0.9))
    # Scale font for many columns
    label_fs = max(8, min(11, int(120 / n)))

    fig, ax = plt.subplots(figsize=(w, CHART_H))
    bars = ax.bar(names, scores, color=colors, width=0.5,
                  edgecolor="white", linewidth=0.5)
    ax.set_ylim(0, 1.22)
    ax.set_title("5. Outlier Detection (Consistency)", loc="left", fontsize=12)
    ...
    # Rotate labels for many columns to prevent overlap
    ...
    for bar, score in zip(bars, scores):
        ax.text(bar.get_x() + bar.get_width() / 2,
                ...
                ha="center", fontsize=max(8, label_fs - 1),
...
# ── 6. Frequency plots — neutral grey ────────────────────────────────────────
def frequency_plots(profile: DatasetProfile, output_dir: str) -> str:
    ...
    for i, col in enumerate(subset):
        ...
        ax.set_title(col.col_name[:20], fontsize=11, fontweight="bold")
        ...
        for bar, v in zip(bars, values):
            ax.text(v + 0.1, bar.get_y() + bar.get_height() / 2,
                    f"{v:,}", va="center", fontsize=10, color=DARK)
