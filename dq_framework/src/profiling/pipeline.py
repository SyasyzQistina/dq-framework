...
Ties together ingestion, schema detection, and the four dimension
functions (dimensions.py) into a single entry point: run(). Given a
file path, returns a fully populated DatasetProfile - one ColumnProfile
per column, each with its dimension scores, descriptive stats, and any
detected QualityIssues.
"""
...
def _clean_name(stem: str) -> str:
    """Convert a file stem into a clean, readable dataset name.

    - Replaces underscores and hyphens with spaces.
    - Strips date-like patterns (e.g. '31-12-2025', '2026-06-11').
    - Strips standalone 4-digit years.
    - Strips common suffixes ('final', 'v1', 'v2', 'clean', 'raw', etc).
    - Collapses extra whitespace and title-cases the result.
    """
    name = stem.replace("_", " ").replace("-", " ")

    # Date patterns: DD MM YYYY or YYYY MM DD (spaces already substituted above)
    ...
    # Common filename suffixes that shouldn't appear in a display name
    name = re.sub(
        r'\b(final|v\d+|with coordinates?|clean|raw)\b',
...
def run(source: str) -> DatasetProfile:
    """Run the full profiling pipeline on a dataset file.

    Steps: load the file -> detect schema -> compute the four quality
    dimensions and descriptive stats per column -> detect issues per
    column -> aggregate dimension scores and build the summary line.
    """

    # STEP 1: Load dataset and detect its schema
    df = load(source)
    ...
    # STEPS 2-5: Profile each column individually
    for col in df.columns:
        ...
        # Descriptive stats - numeric columns get min/max/mean/std;
        # categorical/text columns get their top 5 most frequent values.
        top_values = {}
    ...
    # STEP 6: Aggregate dimension scores across all columns and build
    # the one-line dataset summary shown at the top of the report.
